...
export class AddEditDealComponent implements OnInit {
	...
	private getDealDetail(): void {
		...
		this.dealService.getById(this.dealId).subscribe((res: any) => {
			...
			if (res.response_data.products.length > 0) {
				res.response_data.products.forEach((val) => {
					this.productsId.push(val.productId)
				})
			}
	...
	public checkOperationType(): void {
		...
		if (this.deal.dealType === this.dealTypes["CATEGORY"]) this.deal.products = null;
	...
	private saveDeal(): void {
		if (this.productsId != null) {
			this.productsId.forEach((val) => {
				this.deal.products.push({ productId: val })
			})
		}
